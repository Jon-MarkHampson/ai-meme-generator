...
class ConversationUpdateSignal(BaseModel):
    """
    Real-time signal for conversation state updates.
    
    Sent through the SSE stream to notify the frontend when conversation
    metadata changes, enabling dynamic UI updates without polling.
    
    Attributes:
        type: Signal type identifier for frontend parsing
        conversation_id: ID of the updated conversation
        summary: New conversation summary text
    """
